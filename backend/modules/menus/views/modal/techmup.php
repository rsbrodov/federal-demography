...
?>
<?$menus_dishes =new \common\models\MenusDishes()?>
...
<!--                КОНЕЦ ЗАМЕНЫ-->
                ...
                <td class="text-center"><? $protein = sprintf("%.2f" ,$menus_dishes_model->get_products_bju($d_product->products_id, $id, 'protein') * (($d_product->net_weight)/100) * $indicator); echo $protein; $super_total_protein = $super_total_protein + $protein; ?></td>
                <td class="text-center"><? $fat = sprintf("%.2f" ,$menus_dishes_model->get_products_bju($d_product->products_id, $id, 'fat') * (($d_product->net_weight)/100) * $indicator); echo $fat; $super_total_fat = $super_total_fat + $fat; ?></td>
                <td class="text-center"><? $carbohydrates_total = sprintf("%.2f" ,$menus_dishes_model->get_products_bju($d_product->products_id, $id, 'carbohydrates_total') * (($d_product->net_weight)/100) * $indicator); echo $carbohydrates_total; $super_total_carbohydrates_total = $super_total_carbohydrates_total + $carbohydrates_total; ?></td>
                <td class="text-center"><? $energy_kkal = sprintf("%.1f" ,$menus_dishes_model->get_kkal($d_product->products_id, $id) * (($d_product->net_weight)/100) * $indicator); echo $energy_kkal; $super_total_energy_kkal = $super_total_energy_kkal + $energy_kkal;?></td>
            </tr>
        <?}else{?>
            <tr>
                <td class="text-center"><?=$number_row?></td>
...
<?= Html::a('<span class="glyphicon glyphicon-download"></span> Скачать в PDF технологическую карту', ['techmup-export?menus_dishes_id=' . $id.'&indicator='.$indicator],
    [
        'class'=>'btn btn-white',
