...
$this->title = 'Разработка(редактирование) цикличного меню';
...
?>
    <div class="menus-dishes-index">
        ...
        <? if (empty($my_menus) && (!(Yii::$app->user->can('rospotrebnadzor_camp') || Yii::$app->user->can('rospotrebnadzor_nutrition')))) { ?>
            <p class="text-center" style="color: red"><b>У Вас не созданы меню и не добавлены блюда.(Перейдите в раздел
                    "Архив меню" или "Настройка меню")</b></p>
        <? } ?>
        ...
        if (Yii::$app->user->can('admin'))
        {
            ...
            foreach ($my_sborniks as $my_sbornik) {
                //$my_sbornik_array - ЭТО ТО ЧТО В СТРОКЕ; $my_sbornik_string - ЭТО ТО ЧТО БУДЕТ ВЫПАДАТЬ В БЛЮДАХ
                //ВРЕМЕННО СКРЫВАЕМ ТАК КАК РОМАНЕНКО ПОПРОСИЛ ОСТАВИТЬ ТОЛЬКО 3 СБОРНИКА
                //$my_sbornik_string .= $my_sbornik->id;
                //$my_sbornik_string .= ',';
                //$my_sbornik_array[] = $my_sbornik->id;
            }
            ...
            //ВРЕМЕННО СКРЫВАЕМ ТАК КАК РОМАНЕНКО ПОПРОСИЛ ОСТАВИТЬ ТОЛЬКО 3 СБОРНИКА
            $my_sbornik_string .= '1,4,263';//УДАЛИТЬ
            //$my_sbornik_string .= '1,2,4,263';//ВОССТАНОВИТЬ
            $my_sbornik_array[]=1;
            //$my_sbornik_array[]=2;
            $my_sbornik_array[]=4;
			$my_sbornik_array[]=263;
        ...
        if (Yii::$app->user->can('minobr') || Yii::$app->user->can('subject_minobr'))
        {
            $recipes_collections = RecipesCollection::find()->where(['organization_id' => [7, Yii::$app->user->identity->organization_id]])->andWhere(['!=', 'id', 934])->all();
            $recipes_collection = ArrayHelper::map($recipes_collections, 'id', 'name');
        ...
        //        для организаций в которых есть и школа и детский сад
        if (Yii::$app->user->identity->organization_id == 5949)
        {
            $recipes_collections = RecipesCollection::find()->where(['organization_id' => [7, Yii::$app->user->identity->organization_id]])->all();
            $recipes_collection = ArrayHelper::map($recipes_collections, 'id', 'name');
        ...
        <div class="block container-fluid mt-0 pl-0 pr-0" style="margin-top: 10px;">
            <? echo '<p class="text-center" style="font-size: 26px; font-weight: 500;">' . $nutrition->name . '</p>' ?>
            <table id="nutri_<?php echo $nutrition->id ?>" class="table table-hover table-responsive table-bordered">
                ...
                <tr class="">
                    ...
                    <th class="text-center">Просмотр технологической карты с текущим выходом<i class="fa fa-info-circle"
                                                                                               title="Длительность сезона в месяцах и в днях. Ежемесячный платеж формируется от делеления стоимости сезона на количество месяцев"
                                                                                               data-toggle="tooltip"></i>
                    </th>
                    <th class="text-center">Изменить выход<i class="fa fa-info-circle"
                                                             title="Здесь нужно выбрать месяц, в котором сумма ежемесячного платежа будет уменьшена на сумму первого взноса. Так как участник уже вносил 500 рублей на участие, соответственно один из платежей нужно уменьшить на эту сумму. По умолчанию это первый месяц"
                                                             data-toggle="tooltip"></i></th>
                        ...
                        <tr data-id="<?= $m_dish->id; ?>">
                            ...
                            <td class="check text-center">
                                <?= Html::button('<span class="glyphicon glyphicon-list-alt"></span> из сборника', [
                                    ...
                                    'onclick' => '
                          $.get("../modal/showtechmup?id=" + $(this).attr("data-dishes_id"), function(data){
                          $("#showTechmup .modal-body").empty();
                            ...
                            <td class="check_card text-center">
                                <?= Html::button('<span class="glyphicon glyphicon-list-alt"></span> на ' . $m_dish->yield . ' грамм ', [
                                    ...
                                    'data-id' => $m_dish->id,
                                    'onclick' => '
                          $.get("../modal/showtechmup_current_yield?id=" + $(this).attr("data-id"), function(data){
                          $("#showTechmup .modal-body").empty();
                            $("#showTechmup .modal-body").append(data);
                            //console.log(data);
                            $("#showTechmup").modal("show");
                ...
                <tr id="add_dish_<?php echo $nutrition->id ?>">
                    ...
                    <td colspan="1" class="text-center">
                        <?= Html::button('См. техкарту указанного блюда', [
                            'title' => Yii::t('yii', 'Посмотреть тех. карту'),
                            'data-toggle' => 'tooltip',
                            'data-nutrition_id' => $nutrition->id,
                            'class' => 'btn btn-sm main-button-see',
                            'onclick' => 'console.log($(".dishes_id_" + $(this).attr("data-nutrition_id")).attr("data-dishes"));
                          $.get("../modal/showtechmupadd?id=" + $(".dishes_id_" + $(this).attr("data-nutrition_id")).attr("data-dishes"), function(data){
                          $("#showTechmup .modal-body").empty();
                            $("#showTechmup .modal-body").append(data);
                            //console.log(data);
                            $("#showTechmup").modal("show");
                    ...
                    <td colspan="2"><?= Html::textInput('yield', '', ['placeholder' => "Введите выход блюда", 'class' => 'form-control yield_' . $nutrition->id]); ?></td>
                    <td colspan="2"
                        class="text-center"><?= Html::button('Добавить в меню', ['class' => 'btn main-button-3', 'onclick' => 'saveDish(' . $post['menu_id'] . ',"' . $post['cycle'] . '","' . $post['days_id'] . '","' . $nutrition->id . '")']); ?></td>
    ...
    <div class="text-center">
        <?= Html::button('Посмотреть состав за ' . $nutrition->name, [
            ...
            'data-nutrition_id' => $nutrition->id,
            'onclick' => '
                          $.get("../modal/show_composition?menu_id=" + $(this).attr("data-menu_id") + "&cycle=" + $(this).attr("data-cycle") + "&days_id="  + $(this).attr("data-days_id") + "&nutrition_id=" + $(this).attr("data-nutrition_id"), function(data){
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data) {
                        ...
                        //$('tr[data-id="'+data.id+'"]').find('td.pencil').html('<button type="button" class="btn btn-sm main-button-edit" title="Редактирование" data-toggle="tooltip" onclick="editDishes('+ data.dishes_id+', '+ data.created_at+', '+ data.yield +', '+ data.id +')"><span class="glyphicon glyphicon-pencil"></span></button>');
                        //$('tr[data-id="'+data.id+'"]').find('td.pencil').html('<button type="button" class="btn btn-sm main-button-edit" title="Редактирование" data-toggle="tooltip" onclick="editDishes('+ data.dishes_id+', '+ data.yield +', '+ data.id +')"><span class="glyphicon glyphicon-pencil"></span></button>');
