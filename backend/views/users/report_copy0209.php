...
<? if (!empty($districts)) //нажали кнопку показать
{
    ...
    <table class="table table-bordered table-sm table-responsive">
        ...
        <tr>
            ...
            <th class="text-center" rowspan="1" colspan="6">1 смена</th>
            <th class="text-center" rowspan="1" colspan="6">2 смена</th>
            <th class="text-center" rowspan="1" colspan="6">3 смена</th>
            <th class="text-center" rowspan="1" colspan="6">4 смена</th>
        ...
        <tr>
            ...
            for ($i = 0; $i < 4; $i++)
            {
                ...
                echo '<th class="text-center" rowspan="1" colspan="1">Дата окончания смены</th>';
                echo '<th class="text-center" rowspan="1" colspan="1">Длина смены</th>';
                    ...
                    foreach ($organizations as $organization)
                    {
                        ...
                        for ($j = 1; $j < 5; $j++)
                        {
                            ...
                            if ($fact_inf_camp_smena->start_date == '')
                            {
                                ...
                                $fact_inf_camp_smena_j[$j+5] = 'Фактическая информация не внесена';
                            }
                            else
                            {
                                $fact_inf_camp_smena_j[$j] = $fact_inf_camp_smena->start_date;
                                $fact_inf_camp_smena_j[$j+5] = $fact_inf_camp_smena->expiry_date;
                        ...
                        if($fact_inf_camp_smena_j[6] !== 'Фактическая информация не внесена' || $fact_inf_camp_smena_j[1] !== 'Фактическая информация не внесена')
                        {
                            $table .= '<td colspan="1" class="text-center">' . (ceil((strtotime($fact_inf_camp_smena_j[6]) - strtotime($fact_inf_camp_smena_j[1])) / 86400) + 1) . '</td>';
                        }
                        else {
                            $table .= '<td colspan="1" class="text-center">-</td>';
                        }
...
//
                        ...
                        $table .= '<td colspan="1" class="text-center">' . $fact_inf_camp_smena_j[7] . '</td>';
                        if($fact_inf_camp_smena_j[7] !== 'Фактическая информация не внесена' || $fact_inf_camp_smena_j[2] !== 'Фактическая информация не внесена')
                        {
                            $table .= '<td colspan="1" class="text-center">' . (ceil((strtotime($fact_inf_camp_smena_j[7]) - strtotime($fact_inf_camp_smena_j[2])) / 86400) + 1) . '</td>';
                        }
                        else {
                            $table .= '<td colspan="1" class="text-center">-</td>';
                        }
                        ...
                        $table .= '<td colspan="1" class="text-center">' . $fact_inf_camp_smena_j[8] . '</td>';
                        if($fact_inf_camp_smena_j[8] !== 'Фактическая информация не внесена' || $fact_inf_camp_smena_j[3] !== 'Фактическая информация не внесена')
                        {
                            $table .= '<td colspan="1" class="text-center">' . (ceil((strtotime($fact_inf_camp_smena_j[8]) - strtotime($fact_inf_camp_smena_j[3])) / 86400) + 1) . '</td>';
                        }
                        else {
                            $table .= '<td colspan="1" class="text-center">-</td>';
                        }
                        ...
                        $table .= '<td colspan="1" class="text-center">' . $fact_inf_camp_smena_j[9] . '</td>';
                        if($fact_inf_camp_smena_j[9] !== 'Фактическая информация не внесена' || $fact_inf_camp_smena_j[4] !== 'Фактическая информация не внесена')
                        {
                            $table .= '<td colspan="1" class="text-center">' . (ceil((strtotime($fact_inf_camp_smena_j[9]) - strtotime($fact_inf_camp_smena_j[4])) / 86400) + 1) . '</td>';
                        }
                        else {
                            $table .= '<td colspan="1" class="text-center">-</td>';
                        }
        ...
        $table .= '<td class="text-center align-middle">' . (($i == 0) ? 0 : (count($array_org)-1)) . '</td>';
